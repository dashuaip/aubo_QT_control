...
void Util::initJointAngleArray(double *array, bool unit, double joint0, double joint1, double joint2, double joint3, double joint4, double joint5)
{
    if (unit == RAD){ // 输入的是弧度
        array[0] = joint0;
        array[1] = joint1;
    ...
    if (unit == DEG){ // 输入的是角度
        array[0] = joint0/180.0*M_PI;
        array[1] = joint1/180.0*M_PI;
        array[2] = joint2/180.0*M_PI;
        array[3] = joint3/180.0*M_PI;
        array[4] = joint4/180.0*M_PI;
        array[5] = joint5/180.0*M_PI;
    }
}
